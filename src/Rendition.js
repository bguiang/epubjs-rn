...
class Rendition extends Component {
  ...
  display(target) {
    console.log("Rendition.display(" + target + ")");
    let spine = typeof target === "number" && target;

    if (!this._webviewLoaded) return;

    if (spine) {
      console.log("Send display target to bridge as spine");
      this.sendToBridge("display", [{ spine: spine }]);
    } else if (target) {
      console.log("Send display target to bridge as target");
      this.sendToBridge("display", [{ target: target }]);
    } else {
      console.log("Send display target to bridge as nothing");
